package com.runo.notes.data.source.remote

import com.runo.notes.data.Result
import com.runo.notes.data.Task
import com.runo.notes.data.source.DataSource
...
object TaskRemoteDataSource : DataSource {
    ...
    init {
        addTask(
            "Super Hero",
            "There has been so many abidance for the host and produces different creatures"
        )
        addTask(
            "Indian Programmer",
            "There are so many great coder all over india, they will now create so many different possibilities"
        )
    }
    ...
    override suspend fun completeTask(task: Task) {
        val completeTask =
            Task(task.title, task.description, task.created, task.updated, true, task.entry_id)
